...
// Add pauses at full stops and commas
const addPausesToText = (text) => {
  return text
    .replace(/(\.|\?|\!)\s*/g, "$1 ")  // Add space after punctuation
    .replace(/,/g, ", ");  // Optional: Add space after commas for better pause
};
...
const readAloud = (title, description, content) => {
  ...
  const rawText = `${cleanTitle}. ${cleanDescription}. ${cleanContent}`.trim();

  // Clean text for pauses
  const textToRead = addPausesToText(rawText);
  ...
  speech.lang = "en-US";
  speech.pitch = 1;
  speech.rate = 0.8;  // Adjust the rate to be more natural
  speech.volume = 1;

  // Adding event listener for onboundary to sync with visual text highlighting
  speech.onboundary = (event) => {
    // If you want to visually highlight text, add logic here to update the state of the current word or sentence
    console.log(`Currently reading: ${event.charIndex} - ${event.name}`);
  };
      ...
      <BlogLayout>
        <section className="section">
          ...
          <div className='flex flex-col lg:px-0 py-2 lg:py-2'>
            ...
            <div
              className="htmlContent text-[#242424] lg:text-justify text-base lg:text-[20px] leading-8 lg:leading-9 lg:tracking-wide pt-4 pb-4 px-1 lg:px-0  rounded-lg georgia-font xl:mx-96 lg:mx-56 mx-6"
