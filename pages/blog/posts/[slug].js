...
function SinglePost({ canonicalUrl }) {
  ...
  useEffect(() => {
    // Check if the browser supports speech synthesis natively
    if (window.speechSynthesis) {
      fetchVoices();
      window.speechSynthesis.onvoiceschanged = fetchVoices;
    } else {
      console.warn("speechSynthesis is not supported, using polyfill.");
      
      // If speech synthesis is not available, load the polyfill
      loadPolyfill().then(() => {
        fetchVoices();
        window.speechSynthesis.onvoiceschanged = fetchVoices;
      });
    ...
    // Function to load the polyfill asynchronously
    const loadPolyfill = () => {
      return new Promise((resolve, reject) => {
        const script = document.createElement('script');
        script.src = "https://cdn.jsdelivr.net/npm/speech-synthesis-polyfill@0.3.0/dist/speechSynthesis.min.js";
        script.onload = () => resolve();
        script.onerror = () => reject('Failed to load polyfill');
  ...
  const readAloud = (title, description, content) => {
    ...
    if (femaleVoice) {
      ...
      speech.voice = window.speechSynthesis.getVoices()[0]; // Fallback to first available voice
    }
    speech.lang = "en-US";
    speech.pitch = 1;
    speech.rate = 1;
  ...
  const handleLike = async () => {
    try {
      const q = query(
        collection(fireDb, "blogPost", { canonicalUrl }),
        where("slug", "==", slug)
      );
      const querySnapshot = await getDocs(q);
        ...
        <section className="section">
          <Head>
            ...
            <meta
              ...
              content={postDisplay?.description}
            />
          ...
          <div className="flex flex-col lg:px-0 py-2 lg:py-2">
            <div className="xl:mx-96 lg:mx-56 mx-6 lg:px-0">
              <p className="lg:text-[40px] lg:leading-tight text-2xl font-extrabold lg:py-4 py-2 helvetica-font tracking-tight">
                {postDisplay?.title}
              </p>
            ...
            <div className="">
              ...
              <ul className="py-2 flex  items-center justify-start gap-x-8 text-xs lg:text-base xl:mx-96 lg:mx-56 mx-6 ">
                <li className="flex items-center gap-5">
                  <span>{<p>{StaticData(postDisplay?.time.seconds)}</p>}</span>
                  <p>{postDisplay?.date.slice(0, 12)}</p>
                  {console.log(
                    postDisplay?.date.slice(0, 12),
                    "postDisplay?.date"
                  )}
                </li>
              </ul>
            ...
            <div className="flex gap-8 py-4 border-t-2 border-b-2 xl:mx-96 lg:mx-56 mx-6 px-4 lg:px-0">
              ...
              <p className="flex gap-2">
                ...
                {/* Display comments count from state */}
              </p>
            </div>
            {commentShow && (
              <div className="rounded-sm w-[400px] xl:mx-96 lg:mx-56 mx-6 px-4 lg:px-0">
                ...
                <form
                  className="flex flex-col gap-4 bg-gray-100 border-2 p-4"
              ...
              <div className=" lg:grid-cols-2 grid grid-cols-2 lg:gap-x-8 lg:gap-y-10  gap-7 pt-6  lg:pt-6">
                ...
                {postlist?.length > 0 ? (
                  postlist.slice(0, 4).map((post, i) => (
                    <div
                      key={`key-${i}`}
                      className={"xl:w-[315 px] lg:h-[350px] w-full h-30  "}
                    >
                      <Link
